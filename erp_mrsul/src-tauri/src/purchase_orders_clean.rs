...
fn generate_purchase_order_number(conn: &Connection) -> Result<String, String> {
    ...
    // Verificar se a tabela de numeraÃ§Ã£o existe
    conn.execute(
    ...
    // Inserir configuraÃ§Ã£o inicial se nÃ£o existir
    conn.execute(
    ...
    // Determinar o prÃ³ximo nÃºmero
    let (novo_numero, novo_ano) = if current_year != ano {
        // Mudou o ano, reiniciar numeraÃ§Ã£o
        (1, current_year)
    } else {
        // Mesmo ano, incrementar nÃºmero
        (ultimo_numero + 1, ano)
    };

    // Atualizar a numeraÃ§Ã£o
    conn.execute(
...
pub async fn create_purchase_order(request: CreatePurchaseOrderRequest) -> Result<PurchaseOrder, String> {
    ...
    if fornecedor_exists == 0 {
        return Err("Fornecedor nÃ£o encontrado".to_string());
    }
    ...
    if let Some(ref projeto_id) = request.projeto_id {
        ...
        if projeto_exists == 0 {
            return Err("Projeto nÃ£o encontrado".to_string());
        }
    }
