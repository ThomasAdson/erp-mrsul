...
fn row_to_purchase_order(row: &Row) -> SqliteResult<PurchaseOrder> {
    Ok(PurchaseOrder {
        ...
        fornecedor_id: row.get("fornecedor_id")?, // Read as string directly (now TEXT in DB)
        ...
        comprovante_nome: row.get("comprovante_nome")?,
...
// Tauri Commands
#[command]
pub fn create_purchase_order(request: CreatePurchaseOrderRequest) -> Result<PurchaseOrder, String> {
    ...
    let mut stmt = conn.prepare(
        "SELECT id, numero_pedido, fornecedor_id, data_pedido, 
         ...
         observacoes, comprovante_nome, created_at, updated_at,
...
#[command]
pub fn get_all_purchase_orders() -> Result<Vec<PurchaseOrder>, String> {
    let conn = DB_CONN.lock().map_err(|e| format!("Database lock error: {}", e))?;
...
#[command]
pub fn get_purchase_order_by_id(id: String) -> Result<PurchaseOrder, String> {
    let conn = DB_CONN.lock().map_err(|e| format!("Database lock error: {}", e))?;
...
#[command]
pub fn get_purchase_order_details(id: String) -> Result<PurchaseOrderWithDetails, String> {
    // Buscar o pedido primeiro
    let pedido = get_purchase_order_by_id(id.clone())?;
...
#[command]
pub fn update_purchase_order_status(id: String, new_status: String, observacoes: Option<String>) -> Result<PurchaseOrder, String> {
    {
        let conn = DB_CONN.lock().map_err(|e| format!("Database lock error: {}", e))?;
...
#[command]
pub fn get_purchase_order_items(pedido_id: String) -> Result<Vec<PurchaseOrderItemWithMaterial>, String> {
    let conn = DB_CONN.lock().map_err(|e| format!("Database lock error: {}", e))?;
...
#[command]
pub fn receive_purchase_order_items(
    pedido_id: String,
    nota_fiscal_number: String,
    ...
    let mut stmt = conn.prepare("SELECT status FROM pedidos_compra WHERE id = ?1")
        .map_err(|e| format!("Failed to prepare status check query: {}", e))?;
    ...
    if !["confirmado", "recebido_parcial"].contains(&current_status.as_str()) {
        return Err("Pedido deve estar confirmado para receber itens".to_string());
    }

    drop(stmt); // Drop statement before starting transaction
    ...
    // Atualizar quantidades recebidas dos itens
    {
        let mut stmt = tx.prepare(
            "UPDATE pedidos_compra_itens 
             SET quantidade_recebida = quantidade_recebida + ?1, updated_at = ?2
             WHERE id = ?3"
        ).map_err(|e| format!("Failed to prepare update item query: {}", e))?;
        ...
        for item in &items {
            stmt.execute(params![
                ...
                now,
                item.item_id.parse::<i64>().map_err(|_| "Invalid item ID format".to_string())?
    ...
    let new_status = {
        let mut stmt = tx.prepare(
            "SELECT COUNT(*) as total,
                    SUM(CASE WHEN quantidade_recebida >= quantidade THEN 1 ELSE 0 END) as completos
             FROM pedidos_compra_itens 
             WHERE pedido_id = ?1"
        ).map_err(|e| format!("Failed to prepare status check query: {}", e))?;

        let (total, completos): (i64, i64) = stmt.query_row(params![pedido_id.parse::<i64>().map_err(|_| "Invalid ID format".to_string())?], |row| {
            Ok((row.get("total")?, row.get("completos")?))
        }).map_err(|e| format!("Failed to get item status: {}", e))?;
    ...
    // Atualizar o pedido com novo status e comprovante
    tx.execute(
        "UPDATE pedidos_compra 
         SET status = ?1, comprovante_nome = ?2, updated_at = ?3
         WHERE id = ?4",
        params![
            new_status,
            comprovante_url,
            Utc::now().to_rfc3339(),
            pedido_id.parse::<i64>().map_err(|_| "Invalid ID format".to_string())?
        ]
    ...
    tx.execute(
        ...
        params![
            ...
            current_status,
            new_status,
            format!("Recebimento registrado com NF: {}", nota_fiscal_number),
            Utc::now().to_rfc3339()
        ]
    ).map_err(|e| format!("Failed to create history entry: {}", e))?;
    ...
    // Retornar o pedido atualizado
    get_purchase_order_by_id(pedido_id)
}

#[command]
pub fn update_purchase_order_voucher(order_id: String, voucher_path: String) -> Result<(), String> {
    let conn = DB_CONN.lock().map_err(|e| format!("Database lock error: {}", e))?;
    ...
    conn.execute(
        ...
        params![
            voucher_path,
            Utc::now().to_rfc3339(),
            order_id.parse::<i64>().map_err(|_| "Invalid ID format".to_string())?
        ]
    ).map_err(|e| format!("Failed to update voucher: {}", e))?;
...
pub fn save_purchase_order_voucher(
    ...
    let file_path = vouchers_dir.join(format!("{}_{}", timestamp, file_name));
    ...
    update_purchase_order_voucher(order_id, file_path_str.clone())?;
...
pub fn view_purchase_order_voucher(voucher_path: String) -> Result<(), String> {
    let path = Path::new(&voucher_path);
    if !path.exists() {
...
pub fn download_purchase_order_voucher(voucher_path: String) -> Result<String, String> {
    ...
    let dest_path = downloads_dir.join(file_name.as_ref());
