...
db.all('PRAGMA table_info(pedidos_compra)', (err, rows) => {
  ...
  if (comprovanteField) {
    ...
    db.all('SELECT numero_pedido, comprovante_nome FROM pedidos_compra WHERE comprovante_nome IS NOT NULL AND comprovante_nome != ""', (err, rows) => {
      if (err) {
      ...
      rows.forEach(row => {
        console.log(`Pedido ${row.numero_pedido}: ${row.comprovante_nome}`);
      });
